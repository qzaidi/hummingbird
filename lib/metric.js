var sys = require('sys'),
  path = require('path'),
  fs = require('fs');

var Metric = function() {
  this.name = null;
  this.initialData = {};
  this.interval = 500;
  this.job = null;
  this.minuteJob = null;
  this.clients = [];
  this.incrementCallback = null;
  this.aggregateCallback = null;

  this.resetMinuteData();
  this.resetData();
};

Metric.prototype = {
  init: function(db,visits) {
    var metric = this;

    db.createCollection('metrics', function(err, collection) {
      db.collection('metrics', function(err, collection) {
        metric.collection = collection;
        metric.visits = visits;
        metric.resetData();
        metric.resetMinuteData();
        metric.run();
      });
    });
  },

  run: function() {
    if (this.isDayMetric) {
      this.initDayMetric(this);
    }
   else
    {
      this.job = setInterval(this.runner, this.interval, this);
      this.minuteJob = setInterval(this.minuteRunner, 60000, this);
    }
  },

  runner: function(metric) {
    // NOTE: using 'metric' in place of 'this', since run from setInterval
    if(metric.ignoreOnEmpty && !metric.isDirty) { return; }

    metric.socket.broadcast(JSON.stringify({type: metric.name, data: metric.data}));

    metric.resetData();
  },

  minuteRunner: function(metric) {
    var time = new Date();
    var timestamp = new Date(time.getTime());

    time.setUTCMilliseconds(0);

    time.setUTCSeconds(0);
    var timestampMinute = new Date(time.getTime());

    time.setUTCMinutes(parseInt(time.getUTCMinutes()/30,10)*30);
    var timestampSlice = new Date(time.getTime());

    time.setUTCMinutes(0);
    var timestampHour = new Date(time.getTime());

    time.setUTCHours(0);
    var timestampDay = new Date(time.getTime());

    var mongoData = {
      data: metric.minuteData,
      name: metric.name,
      interval: 60,
      timestamp: timestamp,
      minute: timestampMinute,
      hour: timestampHour,
      day: timestampDay
    };

    sys.log(JSON.stringify(mongoData));

    metric.collection.update({ minute: timestampMinute },
                             { "$inc" : metric.minuteData },
                             { upsert: true },
                             function(err, result) {});

    /* we will keep half hourly slice metrics that makes it possible
     * generate data for any given day for any timezone. Should eventually
     * get rid of hourly metrics, and daily will be generated by mapreduce
     * possibly at the end of day.
     */
    metric.collection.update({ slice: timestampSlice },
                             { "$inc" : metric.minuteData },
                             { upsert: true },
                             function(err, result) {});

    metric.collection.update({ hour: timestampHour },
                             { "$inc" : metric.minuteData },
                             { upsert: true },
                             function(err, result) {});

    metric.collection.update({ day: timestampDay },
                             { "$inc" : metric.minuteData },
                             { upsert: true },
                             function(err, result) {});

    metric.resetMinuteData();
  },

  dayLogger: function(dayData,timestamp) {
    if (dayData) {
      sys.log(sys.inspect(this.dayData));
      this.collection.update({ day: new Date(timestamp) },
			       { "$set": this.dayData },
			       { upsert: true},
			       function(err,result) {});
    }
  },

  removeClient: function(client) {
    this.clients.remove(client);
  },

  addClient: function(client) {
    this.clients.push(client);
  },

  resetData: function() {
    this.data = JSON.parse(JSON.stringify(this.initialData));
    this.isDirty = false;
  },

  resetMinuteData: function() {
    this.minuteData = {};
  },

  stop: function() {
    clearInterval(this.job);
  }
};

Metric.availableMetricPaths = function(callback) {
  var files = fs.readdirSync(path.join(__dirname, 'metrics'));

  for(var i = 0; i < files.length; i++) {
    var file = files[i];
    var filename = file.replace(/\.js$/, '');
    callback(path.join(__dirname, 'metrics', filename));
  }
};

Metric.metrics = [];

Metric.allMetrics = function(callback) {
  if (Metric.metrics.length) {
    for (var i=0; i < Metric.metrics.length; i++)
       callback(Metric.metrics[i]);
    return;
  }

  Metric.availableMetricPaths(function(metricPath) {
    var m = require(metricPath);
    if(typeof(m.canLoad) == "function" && m.canLoad() == false) {
      sys.log("Skipping metric " + m.name + ".");
      return;
    } else {
      sys.log("Loading metric " + m.name + ".");
    }

    // Instantiate a new metric and use the settings from the custom metrics class
    var metric = new Metric()
    for(var key in m) {
      metric[key] = m[key];
    }
    callback(metric);
    Metric.metrics.push(metric);
  });
  sys.log("Loaded " + Metric.metrics.length + " metrics");
};

exports.Metric = Metric;

if (require.main === module) {
  console.log("Metric Test");
  Metric.allMetrics(function(metric) {
			console.log("loaded metric " + metric.name);
		    });
}
